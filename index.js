 ...
 *  Commands: /start /help /balance /start_trade /stop
 *            /profit /status /last_trade /set_risk /set_filter
 ****************************************************************/
...
if (!TELEGRAM_TOKEN || !DERIV_TOKEN || !APP_ID || !RENDER_EXTERNAL_HOSTNAME) {
  console.error("❌ Missing env vars. Check .env on Render.");
  process.exit(1);
}
...
let MAX_TRADES       = 20;   // default risk
...
function updateIndicators(price) {
  emaSeries.push(price);
  rsiSeries.push(price);
  if (emaSeries.length > emaPeriod) emaSeries.shift();
  if (rsiSeries.length > 14)        rsiSeries.shift();
  const ema = emaSeries.length >= emaPeriod
    ? ta.EMA.calculate({ period: emaPeriod, values: emaSeries }).slice(-1)[0]
    : null;
  const rsi = rsiSeries.length >= 14
    ? ta.RSI.calculate({ period: 14, values: rsiSeries }).slice(-1)[0]
    : null;
  return { ema, rsi };
}
...
function handleUpdate(update) {
  ...
  switch (cmd) {
    case "/start":
      return sendTG(chat, "🤖 *B.A.Y.N.E.X* online. Type /help.", true);
    ...
    case "/start_trade":
      ...
      sessionProfit = 0;
      sessionTrades = 0;
      sendTG(chat, "📈 Auto-trading *STARTED*", true);
      startTrader(chat);
      return;

    case "/stop":
      autoTrading = false;
      sendTG(chat, "⏹ Trading *STOPPED*", true);
      return;

    case "/profit":
      return sendTG(chat, `💰 Session profit: *${sessionProfit.toFixed(2)} USD*`, true);

    case "/status":
      return status(chat);

    case "/last_trade":
      return lastTrade(chat);

    case "/set_risk":
      return setRisk(chat, arg);

    case "/set_filter":
      return toggleFilter(chat, arg);
...
function setRisk(id, arg) {
  ...
  MAX_SESSION_LOSS = Number(arg[0]);
  MAX_TRADES       = Number(arg[1]);
  ...
  ws.onmessage = async (ev) => {
    ...
    if (d.msg_type === "tick" && authorized && autoTrading) {
      ...
      try {
        ...
        sendTG(chat, `❌ Trade error: ${e}`);
      }
    }
  };

  ws.onerror = () => {
    sendTG(chat, "📡 Deriv connection error.");
  };

  ws.onclose = () => {
    if (autoTrading) setTimeout(() => startTrader(chat), 5000);
  };
}
/* ── Low-level
